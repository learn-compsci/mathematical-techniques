...
> $$
> 
> So for example, $1 \notin C$, $3 \notin C$. But $\sqrt{2} \in C$, and since $\sqrt{2} \approx 1.414$, $\sqrt{2} \in C$.
...
The power set operation is a little unorthodox, it does not look like a logical operation like the ones we have seen. Given a set $A$, the power set of $A$ is denoted by $\mathcal{P}(A)$ is the set that contains all subsets of $A$. Formally:
